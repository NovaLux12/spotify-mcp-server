...
3. **Rate-limit hold** — after any 429, `_rateLimitUntil` is set to now + `Retry-After` seconds; every later queued request additionally waits out that deadline. The 429 response itself is **retried once** (per call, `retryCount === 0` guard), sleeping the full `Retry-After` first (defaulting to 1 s only when the header is absent — a present-but-unparsable value is a known bug, see #21).
