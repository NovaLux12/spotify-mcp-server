...
async function harness(): Promise<Client> {
  ...
  server.tool('conflict_error', 'conflict', throws(new Error('target playlist changed during union; re-run to review the new destructive impact')));
...
describe('production tool error contract (#921)', () => {
  ...
  it('maps every required failure kind to a structured one-line tool result', async () => {
    ...
    const cases = [
      ...
      { tool: 'conflict_error', kind: 'conflict', status: undefined },
