...
describe('tool surface: annotations', () => {
  ...
  it('keeps the derived-analytics tools off the default surface (#695)', async () => {
    // Spelled out, not imported: the opt-in env var's name is the public
    // contract of #695, and a rename that silently updated both the registrar
    // and this test would keep the two agreeing while the gate stopped
    // matching the documentation.
    const names = (await listTools({})).map((tool) => tool.name);
    const reachable = DERIVED_ANALYTICS_TOOLS.filter((name) => names.includes(name));
