...
function harness(
  ...
  const fakeServer = {
    ...
    ...(elicitResult !== undefined
      ? {
          // Real McpServer shape: both the capability accessor and elicitation
...
describe('add_to_playlist / remove_from_playlist / update_playlist / reorder_playlist_items', () => {
  it('add_to_playlist POSTs uris (and position when given) to /playlists/{id}/items', async () => {
    ...
    // Each mutation is followed by a verification GET — compare writes only.
    const posts = h.client.calls.filter((c) => c.method === 'POST');
    assert.deepEqual(wireCalls(posts).slice(0, 1), [
