    ...
    const result = await this.enqueue(
      async () => {
        ...
        if (res.status === 304) {
          ...
          // held. Report it rather than inventing an empty result, and name
          // the request that got it. The error boundary owns the public
          // vocabulary for this case (`not_modified` / fix "re-read without a
          // validator"), so no reason is set here — a second spelling of one
          // condition is what makes a contract unreadable.
          if (!validator) {
            throw new SpotifyApiError(
              304,
