  ...
  server.prompt(
    ...
    async (rawArgs) => {
      ...
      messages: [{
        ...
        content: {
          type: 'text',
          text: `Catch me up on podcasts since ${args.since} (validate YYYY-MM-DD; if invalid, report the format error and show the expected shape — do not attempt string comparison with a malformed date). Prefer show_new_episodes (since=${args.since}, max_per_show=${args.max_per_show}) to aggregate across saved shows in one call; only fall back to get_saved_shows (fetch_all=true) + list_show_episodes per show if show_new_episodes is unavailable (toolset-trimmed). Keep capped at ${args.max_per_show} per show, newest first. If no saved shows or no episodes since that date, say so explicitly rather than presenting an empty list. For the list_show_episodes fallback, fetch max_results=${Math.max(args.max_per_show * 3, 10)} and keep max_per_show small (default 3) unless I explicitly ask for more. Present grouped by show: episode title, release date, duration, description snippet, and episode URI/ID. Flag anything longer than 90 minutes as a "long listen". At the end ask whether I want any of them queued now via batch_add_to_queue or played directly — do not start playback unprompted. ${STANDARD_FOOTER}`,
        },
      }],
    });
  }
