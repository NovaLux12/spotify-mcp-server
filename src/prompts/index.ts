...
const STANDARD_FOOTER =
  'If any read returns empty (0 tracks/artists/playlists), report it explicitly and suggest a fallback instead of presenting an empty list. If a tool is unavailable (toolset-trimmed) or rate-limited (429), note it with data from spotify://me/rate-limit, wait and retry once before skipping that section — never fail the whole task for one missing step. Validate user-supplied strings (dates as YYYY-MM-DD, playlist names by resolving via get_user_playlists (fetch_all=true) — a name-existence check on a first page of 20 silently duplicates playlists on re-run) and ask for clarification rather than guessing.';
...
export function registerPrompts(server: McpServer): void {
  ...
  server.prompt(
    ...
    async (rawArgs) => {
      ...
      // show_new_episodes has no `since` input: its window is `days` (1–365).
      // Passing a date the tool cannot express silently fell back to the 7-day
      // default, so a "since three months ago" request answered with one week
      ...
      messages: [{
        ...
        content: {
          type: 'text',
          text: `Catch me up on podcasts since ${args.since} (validate YYYY-MM-DD; if invalid, report the format error and show the expected shape — do not attempt string comparison with a malformed date). Prefer show_new_episodes (days=${days}, per_show_limit=${args.max_per_show}) to aggregate across saved shows in one call. show_new_episodes takes no since parameter — its window is days (1–365), so the requested date must be expressed that way.${windowNote} Otherwise fall back to get_saved_shows (fetch_all=true) + list_show_episodes per show. Keep capped at ${args.max_per_show} per show, newest first. If no saved shows or no episodes since that date, say so explicitly rather than presenting an empty list. For the list_show_episodes fallback, fetch max_results=${Math.max(args.max_per_show * 3, 10)} and keep max_per_show small (default 3) unless I explicitly ask for more, then drop any episode released before ${args.since} yourself — that is the only exact-date path. Present grouped by show: episode title, release date, duration, description snippet, and episode URI/ID. Flag anything longer than 90 minutes as a "long listen". At the end ask whether I want any of them queued now via batch_add_to_queue or played directly — do not start playback unprompted. ${STANDARD_FOOTER}`,
        },
      }],
    });
  }
  ...
  server.prompt('crate_digging', 'Crate dig deep cuts for your top artists.', { depth: z.coerce.number().int().positive().max(5).optional().describe('Deep cuts per artist (default 3)') }, async (rawArgs) => ({
    messages: [{ role: 'user', content: { type: 'text', text: `Crate digging: for up to 5 artists from get_top_artists (short_term) — if fewer than 5, dig only the ones available and note the shortfall — find ${(rawArgs as { depth?: number }).depth ?? 3} non-hit deep cuts per artist. Use get_artist_top_tracks to identify hits to skip in one call per artist; it hard-errors with 403 for app registrations created after November 2024, so if it 403s do NOT abandon the tour and do NOT present the artist's biggest songs as deep cuts (the exact inversion of this prompt) — fall back to get_artist_albums + get_album_tracks, treat the earliest-album tracklist plus the absence of those tracks in your saved library and playlists as the popularity proxy, and say in the write-up that hit detection was unavailable. Then use 1–2 broad searches covering multiple artists/genres rather than one per deep cut — search (types=["track"], limit 20) — plus get_artist_albums and get_album_tracks (limit 50, paginate if needed) to surface deep cuts. Propose a playlist with URIs. If I want to keep it, create it with create_playlist and add_to_playlist — preview with dry_run=true before committing. If per-artist searches return 0 deep cuts, report it and suggest broadening. ${STANDARD_FOOTER}` } }],
  }));
        ...
        messages: [{
          ...
          content: {
            ...
            text: [
              ...
              `4. Once approved, check get_user_playlists (fetch_all=true) for existing names to reuse — that call defaults to a single 20-item page, so on an account with more than 20 playlists a first-page check misses an existing bucket playlist and creates a duplicate ("Liked · 2010s" twice) on every re-run; if the listing footer says more playlists exist, keep paging before deciding a name is free. Create any genuinely missing playlists with create_playlist, then fill them with add_to_playlist in batches of at most ${Math.min(args.batch_size, 100)} URIs.`,
              guard,
