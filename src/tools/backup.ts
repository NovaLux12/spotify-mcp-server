...
export function registerBackupTools(server: McpServer, client: SpotifyClient): void {
  ...
  server.tool(
    ...
    async (args) => {
      ...
      // Re-read the store rather than decrementing a counter: the envelope
      // the caller sees is what is actually left on disk.
      const remaining = storeEnvelope(await readStoreEntries(dir), retentionDays);
      return shapeResult(
        args.response_format,
        `Deleted backup ${resolved.file} (${formatBytes(st.size)})${sidecarDeleted ? ' and its metadata sidecar' : ''}. ${describeEnvelope(remaining, retentionDays)}`,
        {
          ok: true,
          deleted: true,
          dry_run: false,
