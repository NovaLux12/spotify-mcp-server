...
async function fetchPlaylistItems(client: SpotifyClient, playlistRef: string): Promise<PlaylistItemObject[]> { const id = encodeURIComponent(normalizePlaylistReference(playlistRef)); return client.getAllPages<PlaylistItemObject>(`/playlists/${id}/items`, { limit: '100' }, { maxItems: FETCH_ALL_CAP() }); }
