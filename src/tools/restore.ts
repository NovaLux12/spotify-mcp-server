...
export function registerRestoreTools(server: McpServer, client: SpotifyClient): void {
  server.tool(
    ...
    "STRICTLY ADDITIVE restore of a library snapshot written by backup_library_snapshot. Adds what is missing: saves tracks/albums/shows/episodes/audiobooks, follows artists, and creates NEW playlists named 'Restored · <name> (<snapshot date>)' — existing playlists are never touched and nothing is ever deleted, renamed, or overwritten. Truncated snapshots are previewable but refused before confirmation or writes; quota-hit, contentless or wrong-schema_version snapshots are refused outright. dry_run defaults to TRUE (read-only preview); dry_run=false requires explicit confirmation before any write and fails closed when elicitation is unavailable or errors (SPOTIFY_MCP_CONFIRM=never bypasses).",
    {
      backup_path: z
        .string()
        .min(1)
