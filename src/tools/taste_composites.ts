/**
 * Wave-2 taste composites: 10 composite tools over the stats.fm PUBLIC API v1
 * (no auth), shaping taste data into playlist specs, briefs, and reports. All
 * ten are read-only and never touch the Spotify client.
 *
 * The eleventh, `taste_to_playlist`, is the one writer (`dry_run=false`
 * creates the playlist and adds the tracks) and moved to taste_playlist.ts
 * (#1009): the manifest's `readOnlySafe` flag is per registrar row, so a
 * writer sharing this row is either exposed to read-only sessions or drags
 * these ten reads out with it.
 *
...
export function picksFromTopTracks(payload: unknown, evidence: string): TrackPick[] {
...
export function picksFromStreams(streams: TasteStream[], evidence: string): TrackPick[] {
...
export function renderPicks(picks: TrackPick[]): { lines: string[]; missing: string[] } {
  const lines: string[] = [];
...
// ---------------------------------------------------------------------------
// Registration — 10 composite tools, all taste_ prefixed, all read-only over
// the public stats.fm API. The writer (taste_to_playlist) lives in
