...
| `~/.spotify-mcp/receipts.jsonl` (`SPOTIFY_MCP_RECEIPTS_DIR`, else `SPOTIFY_MCP_HISTORY_DIR`) | Mutation receipts: receipt id, kind, playlist id, the mutated item URIs, per-uri occurrence counts, the playlist row positions a mutation created, the issue timestamp, and the verified/missing outcome. These are the anchors `verify_receipt`, `receipt_lookup` and `undo_mutation` resolve. | Opt-in with `SPOTIFY_MCP_RECEIPTS`; appended for each mutation and reloaded on startup so receipts survive a restart. Retention: the newest 100 receipts, each for at most `SPOTIFY_MCP_RECEIPTS_TTL_HOURS` (default 24; `0` disables expiry), evicted oldest-first. With the flag unset nothing is written and the in-memory-only store disappears when the process exits. Delete the file to remove every persisted receipt. |
| `~/.spotify-mcp/backups/*.json` (`SPOTIFY_MCP_BACKUP_DIR`) | Explicit library backups: saved tracks, albums, shows, episodes, audiobooks, followed artists, playlists, item URIs/names, counts, timestamps, and optional notes. Each file is stamped `_meta.spotify_data: true` and `_meta.retention_until`. | Written only by backup tools. Expired automatically: `backup_library` and `list_backups` delete snapshots older than `SPOTIFY_MCP_BACKUP_RETENTION_DAYS` (default 30; `0` disables expiry; the smallest enabled window is 1 day) and report what was removed. `delete_backup` removes a single snapshot and its sidecar — it is confirmation-gated and previews by default. Delete the directory to remove every backup. |
...
| `~/.spotify-mcp/artist-watchlist.json` (or `SPOTIFY_MCP_DATA_DIR/artist-watchlist.json`) | Watched artist IDs, watchlist creation/check timestamps, and seen release identifiers. | Written by artist-watch tools. A pre-v2 `./data/artist-watchlist.json` is read once and migrated on the next write; a file that cannot be parsed is copied to `<file>.corrupt` and left in place. Delete the file or configured data directory to remove it. |

`SPOTIFY_MCP_DATA_DIR` can also redirect playlist-health snapshots; the default remains `~/.spotify-mcp/playlist-snapshots`. `SPOTIFY_MCP_PORTABILITY_DIR` redirects both portability and listening-history exports. The mutation receipt store keeps the 100 most recent receipts with FIFO eviction; it is process-local and disappears when the process exits unless `SPOTIFY_MCP_RECEIPTS` opts into the `receipts.jsonl` trail described above, which is bounded to those same 100 receipts plus a 24-hour lifetime by default. Receipt ids are scoped to the process that issued them, so an id from an earlier session never resolves to a different mutation.
...
Library backups are the one store with automatic age-based expiry: `backup_library` and `list_backups` delete snapshots older than `SPOTIFY_MCP_BACKUP_RETENTION_DAYS` (default 30 days; set `0` to disable expiry, and note that the smallest enabled window is 1 day) and report the paths, ages, and freed bytes of what they removed, while `list_backups` reports the store envelope (`dir_bytes`, `oldest_created`, per-snapshot `retention_until`). `delete_backup` removes a single snapshot on request: it previews by default (`dry_run` defaults to true), requires explicit confirmation before deleting, refuses any path outside the backup directory, and is irreversible once confirmed. Expiry is judged from each file's recorded `_meta.created`, not its modification time, and a file whose date cannot be read is kept rather than deleted.

The other durable local stores in this release have no automatic age-based expiry, so their retention remains user- and operator-controlled: use the smallest scopes, enable only needed tools, keep exports/snapshots only as long as needed, and delete files when the purpose ends. The default `SPOTIFY_MCP_MAX_ITEMS` is 50, `SPOTIFY_MCP_FETCH_ALL_CAP` is 500, and individual result limits may be lower; these are collection/result caps, not a promise that all data is anonymized.
