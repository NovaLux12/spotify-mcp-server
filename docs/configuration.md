...
| `SPOTIFY_MCP_READONLY` | unset | `1`, `true`, or `yes` hides Spotify-mutating registration modules. Read-only modules, resources, and prompts remain subject to their normal gates. |
...
`SPOTIFY_MCP_READONLY=1` (also `true` or `yes`) prevents registration of Spotify-mutating modules such as playback and scenes, playlist and library mutations, following, users, audiobooks, and destructive helpers. It does **not** imply that every remaining tool is side-effect-free: local-only tools such as the taste feedback store remain available. It also does not bypass the independent toolset, registration-key, or scope gates. Read-only resources and prompts remain available when their own gates permit.
