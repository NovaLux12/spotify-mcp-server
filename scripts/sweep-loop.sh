...
#                clamped to 50, the largest page the Web API honours.
...
#   3  MAX_BATCHES reached; the sweep is incomplete but resumable — re-run the
#      same command later and it continues from the report
#   4  another sweep loop already holds the lock for this report
#   5  three batches running failed to deliver a report: the gauntlet died
#      mid-write, or exited non-zero without reaching its own end-of-batch,
#      quota-wall or completion paths
set -euo pipefail
cd "$(dirname "$0")/.."
...
usage: BATCH=40 INTERVAL=1800 MAX_BATCHES=30 REPORT=path scripts/sweep-loop.sh
  ...
  BATCH        integer >= 1     calls per run (default 40, clamped to 50)
...
exit codes: 0 sweep complete, 2 bad input, 3 max batches reached (resumable),
4 lock held by another loop, 5 the gauntlet kept failing
USAGE
}
...
require_int() {
  ...
  [[ $value =~ ^[0-9]+$ ]] || bad_input "$name must be a whole number, got '$value'"
  if (( value < min )); then
    bad_input "$name must be at least $min, got '$value'"
  fi
  if (( value > max )); then
...
for i in $(seq 1 "$MAX_BATCHES"); do
  ...
  # and three of those in a row is a failure rather than a slow sweep.
  if (( code != 0 )) && ! grep -q 'batch run finished' "$batch_log"; then
    hard_failures=$(( hard_failures + 1 ))
  fi
