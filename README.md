...
154 tools across 38 tool modules, plus the top-level `verify_receipt` diagnostic:

**Playback & devices (33 tools)** — now playing / currently-playing polls, play (by URI, or `play_from_search` to play straight from a name), pause, skip, previous, seek, volume, shuffle, repeat, queue view/add, device list, transfer playback, and `handoff` to move mid-track playback to another Spotify Connect device; plus `queue_playlist` (queue whole playlists/albums/artists in one call), queue reorder/remove/clear stubs, and playback extensions (`save_playback_state`/`restore`/`list`, device rename + volume presets, `tag_listening_session`/`replay`/`list_sessions`, smart-rule persist + show-digest).
...
**Search & catalog (31 tools)** — unified `search` across tracks/artists/albums/playlists/shows/episodes/audiobooks (limit capped at 10 per Spotify's Feb 2026 change) and `search_deep`, which walks past that cap server-side (up to 5 pages of 10 results per type, deduplicated); deep lookups for tracks, artists, artist albums, albums, album tracks, shows, show episodes, episodes, and your profile (`get_me`); batch `get_several_*` lookups for tracks, albums, artists, episodes, shows, audiobooks and chapters, plus artist top-tracks / available-markets — these were removed by Spotify's February 2026 Web API changes and fail with an explanatory message on newer app registrations (they still work with credentials from a grandfathered pre-Nov-2024 app); plus `get_artist_genres`/`get_categories`/`get_category_playlists`, `get_artist_discography`/`resolve_artist`/`save_artist_new_releases` + artist watchlist (`watch_artists`/`check_artist_releases`/`artist_release_digest`), and `search_history`/`search_rerun` with 90-day expiry.
...
**Library (26 tools)** — saved tracks/albums/shows/episodes with optional full pagination; save/remove/check partitioned by type to `/me/tracks|albums|shows|episodes(/contains)` (bare IDs), plus unified `/me/library` tools accepting any mix of track/album/show/episode/audiobook URIs in one request; `library_hygiene` flags incomplete albums and consolidation candidates over your liked tracks; genre tools (`library_genre_report`, `filter_by_genre`, `tag_management`) aggregate saved items by user-declared artist tags kept in a local sidecar; `find_duplicate_saved_tracks` flags double-saves and remasters across your liked tracks (optional `playlist_id` cross-reference marks which duplicates are already curated into a given playlist); `backup_library` / `list_backups` / `restore_library_snapshot` take strictly-additive JSON snapshots of your saved library to a local sidecar and put them back later; plus `library_coverage_report` (orphans/coverage), `listening_heatmap` (168-bucket hour×day), `library_growth_report`, `genre_trends_over_time`, and episode hygiene (`archive_played_episodes`, `mark_episode_played`).

**Playlists (35 tools)** — full CRUD plus item management (add/remove/reorder/replace), cover art retrieval and custom cover upload (`ugc-image-upload` scope required for upload), and `find_duplicates_in_playlist`; power ops: `merge_playlists` (dedupe + append/create), `diff_playlists` (fully paged A/B comparison with position-change detection), `overlap_playlists` (tracks shared across playlists); `export_playlist` dumps any playlist as extended M3U or RFC-4180 CSV (file or inline) and `import_playlist` parses such a document back into a target playlist in deduped batches — a complete migration/backup round trip; `remove_duplicate_playlist_items` cleans the duplicates `find_duplicates_in_playlist` finds in one safe call (keeps first occurrence, dry_run + elicitation for bulk, post-clean re-scan) and `clean_all_playlists` scales that to every playlist after one global confirmation; `create_smart_playlist` builds a playlist from rules over your own listening data (`top_tracks`/`recently_played`/`saved_tracks`, artist filtering, one-per-artist); `grow_playlist`, which proposes tracks using only your own listening data (co-occurrence across your other playlists — no deprecated recommendations endpoint); plus health (`playlist_health_check`, `get_playlist_followers`, `playlist_collaboration_report`, `snapshot_playlist`/`diff_since_snapshot`), batch (`batch_add_to_playlist`, `copy_playlist`, `move_items_between_playlists`), misc (`pin_playlist`/`unpin_playlist`, `playlist_template_apply`), and discovery saves (`save_discover_weekly`/`save_release_radar`) + full library export (`export_library_json`, `export_followed_artists`).
...
**Podcast sessions & show radar (3 tools)** — `plan_podcast_session` greedy-packs your saved episodes into a session of a given length (skipping fully played ones, resuming partially played ones at their position); `start_podcast_session` queues it on a device; and `show_new_episodes` scans every saved show for episodes released within a lookback window, marking which are already saved.
...
Newer modules ride their parent key rather than adding new ones: `listening_report` under `personalization`; scenes/wind-down/queue/playback-ext under `playback`; `search_deep` + browse/artistwatch/search-history under `search`/`catalog`; the audiobook copilot under `audiobooks`; freshness under `following`; merge/diff/overlap/DNA/export/import/smart/clean_all/health/batch/misc/templates/weekly under `playlists`; and library hygiene/dedupe/genre insights/podcast sessions/show radar/receipts/backups/analytics/portability/episodes under `library`. `spotify_doctor` is unconditional — it always registers so diagnostics survive any trim.
