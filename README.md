...
97 tools across 20 tool modules, plus the top-level `verify_receipt` diagnostic:
...
**Playlists (18 tools)** — full CRUD plus item management (add/remove/reorder/replace), cover art retrieval and custom cover upload (`ugc-image-upload` scope required for upload), and `find_duplicates_in_playlist`; power ops: `merge_playlists` (dedupe + append/create), `diff_playlists` (fully paged A/B comparison with position-change detection), `overlap_playlists` (tracks shared across playlists); `export_playlist` dumps any playlist as extended M3U or RFC-4180 CSV (file or inline) and `import_playlist` parses such a document back into a target playlist in deduped batches — a complete migration/backup round trip; and `grow_playlist`, which proposes tracks using only your own listening data (co-occurrence across your other playlists — no deprecated recommendations endpoint).
