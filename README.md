...
98 tools across 20 tool modules, plus the top-level `verify_receipt` diagnostic:
...
Newer modules ride their parent key rather than adding new ones: `listening_report` under `personalization`, scenes/wind-down under `playback`, `search_deep` under `search`, the audiobook copilot under `audiobooks`, freshness under `following`, merge/diff/overlap/DNA/export/import under `playlists`, and library hygiene/dedupe/genre insights/podcast sessions/receipts/backups under `library`. `spotify_doctor` is unconditional — it always registers so diagnostics survive any trim.
